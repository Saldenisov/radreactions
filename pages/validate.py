...
from auth import check_authentication, show_login_page, show_user_profile_page
...
st.set_page_config(layout="wide", page_title="Validate OCR")

# === CONSOLE DEBUG ===
print(f"[VALIDATE PAGE] Starting validation page load")
print(f"[VALIDATE PAGE] Session state keys: {list(st.session_state.keys())}")
print(f"[VALIDATE PAGE] authenticated_user: {st.session_state.get('authenticated_user', 'NOT_SET')}")
print(f"[VALIDATE PAGE] authentication_status: {st.session_state.get('authentication_status', 'NOT_SET')}")

# === DEBUG SECTION (temporary) ===
with st.expander("🐛 Debug Info - Authentication Status"):
    st.write(f"**Session state keys:** {list(st.session_state.keys())}")
    st.write(f"**authenticated_user:** {st.session_state.get('authenticated_user', 'NOT_SET')}")
    st.write(f"**authentication_status:** {st.session_state.get('authentication_status', 'NOT_SET')}")

# === Authentication Check ===
print(f"[VALIDATE PAGE] About to call check_authentication()")
current_user = check_authentication()
print(f"[VALIDATE PAGE] check_authentication() returned: {current_user}")

if not current_user:
    print(f"[VALIDATE PAGE] Authentication failed - showing login page")
    st.error("❌ **Authentication Required**")
    st.info("You are not logged in. Please go back to the main page and log in first.")
    st.markdown("[⬅️ Back to Main Page](/)") 
    show_login_page()
    st.stop()
else:
    print(f"[VALIDATE PAGE] Authentication successful for user: {current_user}")
